...
# Trimmed at the ends and lowercased, because resolveEnvironment() does exactly
# that before its lookup. An interior space is left alone so it still fails in
# both: the point is that the two agree, not that either is lenient.
BG_ENV=$(printf '%s' "${BG_ENV:-}" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//' | tr 'A-Z' 'a-z')
