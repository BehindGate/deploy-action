...
# and the host the CLI is downloaded from.
#
# VALIDATED, NEVER NORMALISED. GitLab minted `aud` from the raw input when it
# expanded the configuration, long before this shell ran, so tidying the value
# here cannot change what the token already claims -- it would only make the
# endpoint disagree with the audience while looking tolerant about it. An
# audience is compared as an exact string, so a value that is not already
# exactly an origin has to be refused rather than repaired.
BG_APP_ORIGIN="${BG_APP_ORIGIN:-}"
[ -n "$BG_APP_ORIGIN" ] || BG_APP_ORIGIN="$BG_DEFAULT_ORIGIN"

case "$BG_APP_ORIGIN" in
  https://*/*)
    bg_fail \
      "The \`app-origin\` input ($BG_APP_ORIGIN) is not a bare origin." \
      '' \
      'It names the BehindGate instance as scheme and host only -- no path, and' \
      'no trailing slash. For example https://app.behindgate.com.' \
      '' \
      'This value is also the audience GitLab minted this job'\''s OIDC token for,' \
      'and an audience is compared as an exact string. A trailing slash is a' \
      'different audience rather than a tidier spelling of the same one, so it' \
      'is refused here instead of being trimmed -- trimming it now could not' \
      'change the token that was already minted.' \
      '' \
      "The deploy endpoint is derived as <app-origin>$BG_DEPLOY_PATH; to pin a" \
      'different endpoint, use the `url` input.'
    ;;
  https://?*) ;;
  *)
    # https only, and http:// is refused rather than warned about: this value is
    # the audience for a bearer credential. To reach a local instance, override
    # `url` and `download-base-url` and use a deploy token instead.
    bg_fail \
      "The \`app-origin\` input ($BG_APP_ORIGIN) is not an https:// origin." \
      '' \
