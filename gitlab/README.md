...
| `tags` | *(none)* | Extra labels for the release, one per line, as `name=value` or a bare `name`. |
...
Every release is tagged with the pipeline's own facts: `sha`, `pipeline` and
`ref`. The workspace shows them in its deploy history, so a release can be traced
back to the commit and the run that published it.

`tags` adds to them, one per line, as `name=value` or a bare `name`:
...
A name starts with a letter and holds letters, digits, dot, hyphen or underscore,
and may appear once. Naming `sha`, `pipeline` or `ref` there replaces the
built-in of that name rather than failing on the repeat.
...
This project is a mirror. The component is written and reviewed in
[github.com/behindgate/deploy-action](https://github.com/behindgate/deploy-action)
under `gitlab/`, and a workflow there pushes that directory here on every merge
to `main`. The GitHub Action wraps the same `bg-deploy` CLI, so a change to the
CLI's contract, a new input or a new pinned version is made once, beside it.

Editing this project directly is not wrong, but the next sync overwrites it.
Open a pull request against `deploy-action` instead. Tags and releases are made
here: the CI/CD catalog lists this project, and the sync never tags.
