...
describe('describeExitCode', () => {
  ...
  test('exit 2 with a token leads with the endpoint mismatch', () => {
    const { detail } = describeExitCode(EXIT_CONFIG, { usesToken: true });
  ...
  // The endpoint is always pinned, so the mismatch is the likeliest cause -- but
  // a token that has simply stopped working produces the same exit code, and
  // describing that as a mismatch sends the reader after the wrong thing.
  test('exit 2 with a token still names an expired or revoked credential', () => {
    const { detail } = describeExitCode(EXIT_CONFIG, { usesToken: true });
    assert.match(detail, /expired or been revoked/);
    assert.match(detail, /Deploy tokens/);
  });
  ...
  test('exit 2 in CI-trust mode points at the trust, not at a secret', () => {
    const { detail } = describeExitCode(EXIT_CONFIG, { usesToken: false });
